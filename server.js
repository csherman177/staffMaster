...
const addRoles = () => {
    inquirer.prompt([
        ...
        .then(answer => {
            ...
            db.query(sql, [answer.addTitle, answer.addSalary, answer.addDepartment], (err, result) => {
                if (err) throw err;
                console.log('Your Department Has Been Added');
...
const updateEmployees = () => {
    ...
    db.query(empSQL, (err, data) => {
        ...
        const employees = data.map((person) =>({name:`${person.first_name} ${person.last_name}`, value:person.id}))
        inquirer.prompt([
            {
                type: 'list',
                name: 'name',
